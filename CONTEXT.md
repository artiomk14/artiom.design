...
*Figma `content-item` (124:655) holds each gem (20px sides, 128px top, 144px bottom; height follows the gem’s max). Gems rest at `scale(0.85)` and return to `1` on holder hover/focus (`t-gem-scale`). First gem is the WC26 walkthrough: complete step, then Reset (`class=neutral`) in the gray holder. Remaining slots stay empty. Next: more Gems.*
...
*Last updated: 2026-08-24 (Gems rest at 0.85, grow on content-item hover)*
